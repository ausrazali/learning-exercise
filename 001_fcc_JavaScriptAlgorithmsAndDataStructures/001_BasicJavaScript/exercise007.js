...
const recordCollection = {
  ...
  function updateRecords(records, id, prop, value) {
    ...
    } else if (prop !== "tracks"){ // and value not empty string
        records[id][prop] = value;
    } else { // prop is tracks and value not empty string 
        if (!records[id].hasOwnProperty("tracks")) {
            records[id]["tracks"] = [];
        }
        records[id]["tracks"].push(value); // .push() add to array's last item
    }
    return records;
  }
